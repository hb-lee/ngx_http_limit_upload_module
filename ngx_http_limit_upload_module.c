...
static ngx_int_t
ngx_http_limit_upload_input_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    off_t                          excess;
    ngx_int_t                      rc;
    ngx_msec_t                     delay;
    ...
    if (ctx->limit_rate) {
        if (in != NULL) {
            ctx->received += ngx_buf_size(in->buf);
            // use the real time not the cached time
...
ngx_http_limit_upload_init(ngx_conf_t *cf)
{
    ...
    *h = ngx_http_limit_upload_handler;
    ...
    ngx_http_top_request_body_filter = ngx_http_limit_upload_input_body_filter;
